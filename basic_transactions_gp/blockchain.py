...
def receive_transaction():
    ...
    if not data['sender'] or not data['recipient'] or not data['amount']:
        response = {
            'message': 'Request must contain sender, recipient, and amount'
        }
        return jsonify(response), 400

    index = blockchain.new_transaction(data['sender'], data['recipient'], data['amount'])

    response = {"message": f"Transacton will be added to block {index}"}
...
def mine():
    ...
    if not data['proof'] or not data['id']:
        response = {
            'message': 'Request must contain proof and id'
        }
        return jsonify(response), 400
    ...
    if is_valid_proof == True:
        ...
        resonse = {
            'message': 'proof not valid or late'
        }
        return jsonify(response), 200
